...
class Problem:
    ...
    category: str      # primary category (Chinese), see LEETCODE_TAG_TO_CATEGORY


# Map a LeetCode topic tag (English) to a Chinese category used in the README.
# Order of priority for picking a single category: see PRIMARY_CATEGORY_PRIORITY.
LEETCODE_TAG_TO_CATEGORY: dict[str, str] = {
    "Array": "数组",
    "String": "字符串",
    "String Matching": "字符串",
    "Hash Table": "哈希表",
    ...
    "Topological Sort": "图",
    "Shortest Path": "图",
    # Specific string algorithms
    "Manacher": "字符串",
    "Z Algorithm": "字符串",
    "Knuth-Morris-Pratt Algorithm": "字符串",
    "Boyer-Moore String-Search Algorithm": "字符串",
}

# Order in which we try to pick a category from a problem's tags list.
# Earlier entries win when a problem carries multiple tags.
PRIMARY_CATEGORY_PRIORITY: list[str] = [
    "数组", "字符串", "链表", "树", "图", "栈", "队列", "堆",
    "哈希表", "双指针", "滑动窗口", "前缀和", "二分查找", "排序",
    "动态规划", "贪心", "回溯", "递归",
    "数学", "位运算", "模拟", "矩阵",
    "线段树", "树状数组", "字典树", "并查集", "分治",
    "迭代器",
...
CATEGORY_ORDER = [
    "数组", "字符串", "哈希表", "链表", "树", "栈", "队列", "堆",
    "数学", "位运算", "双指针", "排序", "二分查找", "滑动窗口",
    "前缀和", "动态规划", "贪心", "回溯", "递归",
    "模拟", "矩阵", "图", "线段树", "树状数组", "字典树",
    "并查集", "分治", "迭代器",
    "其他",
]

# Path to the cached tags map (generated by scripts/fetch_tags.py).
TAGS_PATH = REPO_ROOT / "scripts" / "problems_tags.json"


def _load_tags() -> dict[str, list[str]]:
    """Load {questionFrontendId: [tag_name, ...]} from cache. Empty if missing."""
...
def category_for(num: str, tags: dict[str, list[str]]) -> str:
    """Pick a primary Chinese category for a problem using the priority order."""
    if not num.isdigit() or num not in tags:
        return "其他"
    available_cats: set[str] = set()
    for eng in tags[num]:
        cat = LEETCODE_TAG_TO_CATEGORY.get(eng)
        if cat:
...
TAGS: dict[str, list[str]] = _load_tags()
...
def _parse_problem_dir(entry: Path, name: str) -> Problem | None:
    ...
    category = category_for(num, TAGS)
